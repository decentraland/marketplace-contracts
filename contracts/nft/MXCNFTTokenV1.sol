...
contract  MXCNFTTokenV1  is ERC721Enumerable, Ownable, IMXCRoyaltyStandardV1
{
    ...
    function royaltyInfo(uint256 _tokenId, uint256 _salePrice) external view returns (address receiver, uint256 royaltyAmount, address royaltyRecipient){
        require(_exists(_tokenId), "Royalty: Nonexistent token");
        require(royalties[_tokenId].wantRoyalties == true, "Royalty: Token doesn't want royalties");
        return (royalties[_tokenId].firstOwner, _salePrice.mul(royalties[_tokenId].percentage).div(100), royalties[_tokenId].royaltyRecipient);
    }
